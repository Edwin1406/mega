...
async function cargarDatos() {
  try {
    ...
    // Obtener gramajes, anchos y líneas para llenar los filtros
    const gramajes = new Set();
    const anchos = new Set();
    const lineas = new Set();

    data.forEach(item => {
      if (item.gramaje) gramajes.add(item.gramaje);
      if (item.ancho) anchos.add(item.ancho);
      if (item.linea) lineas.add(item.linea);
    });

    // Llenar los filtros dinámicamente
    const gramajeSelect = document.getElementById("gramaje");
...
function filtrarDatos() {
  const gramajeFiltro = document.getElementById("gramaje").value;
  const anchoFiltro = document.getElementById("ancho").value;
  const lineaFiltro = document.getElementById("linea").value;

  // Filtrar los datos según los filtros seleccionados
  const dataFiltrada = datosOriginales.filter(item => {
    return (
      (gramajeFiltro === "" || item.gramaje === gramajeFiltro) &&
      (anchoFiltro === "" || item.ancho === anchoFiltro) &&
      (lineaFiltro === "" || item.linea === lineaFiltro)
    );
  });
...
// Mostrar los resultados en la tabla de ingresos filtrados
function mostrarResultados(dataFiltrada) {
  // Ocultar la tabla de gramajes
  document.getElementById("tabla-gramajes").style.display = 'none';

  const tbodyIngresos = document.querySelector("#tabla-ingresos tbody");
  tbodyIngresos.innerHTML = '';  // Limpiar tabla de ingresos

  let totalFiltrado = 0;
  dataFiltrada.forEach(item => {
    const fila = document.createElement('tr');
    const celdaGramaje = document.createElement('td');
    celdaGramaje.textContent = item.gramaje;
    const celdaTotal = document.createElement('td');
    const cantidad = parseFloat(item.cantidad.toString().replace(',', '').replace(' ', '')) || 0;
    totalFiltrado += cantidad;
    celdaTotal.textContent = cantidad.toFixed(3);
    fila.appendChild(celdaGramaje);
    fila.appendChild(celdaTotal);
    tbodyIngresos.appendChild(fila);
  });

  // Mostrar el total filtrado
  const totalRow = document.createElement('tr');
  totalRow.classList.add('total-row');
  const celdaTotal = document.createElement('td');
  celdaTotal.colSpan = 2;
  celdaTotal.textContent = `Total: ${totalFiltrado.toFixed(3)}`;
  totalRow.appendChild(celdaTotal);
  tbodyIngresos.appendChild(totalRow);
}
